...
describe ('Salvar todas as receitas de uma vez do array', () => {
    context('Quando informo todas as informações de um array de receitas', () => {
        it ('Então todas as informações são salvas corretamente', (done) => {
            request
            .post(getEnvString('FINANCIAL_INCOME') + 'many')
            .send(finance)
            .end((err, res) => {
